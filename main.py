...
CAT_FRIENDS = "━━ 💗 Amigos ━━"
CAT_MC = "━━ ⛏️ Minecraft ━━"
CAT_ARK = "━━ 🦖・DODÓFILOS | ARK ━━"
CAT_GTA = "━━ 🚗 GTA ━━"
CAT_BOTS = "━━ 🤖 Bots ━━"
CAT_VIP = "━━ 💎 VIP LOUNGE ━━"
CAT_STAFF = "━━ 🛡️・𝙎𝙏𝘼𝙁𝙁 ━━"

CH_WELCOME = "👋・𝙗𝙞𝙚𝙣𝙫𝙚𝙣𝙞𝙙𝙖"
CH_RULES = "📜・𝙧𝙚𝙜𝙡𝙖𝙨-𝙮-𝙧𝙤𝙡𝙚𝙨"
CH_SOCIALS = "🔴・𝙧𝙚𝙙𝙚𝙨-𝙮-𝙙𝙞𝙧𝙚𝙘𝙩𝙤𝙨"
CH_LIVE = "🔴・𝙧𝙚𝙙𝙚𝙨-𝙮-𝙙𝙞𝙧𝙚𝙘𝙩𝙤𝙨"
CH_ANNOUNCE = "📢・𝙖𝙣𝙪𝙣𝙘𝙞𝙤𝙨"
CH_ROLES = "📜・𝙧𝙚𝙜𝙡𝙖𝙨-𝙮-𝙧𝙤𝙡𝙚𝙨"

CH_CHAT = "💬・𝙘𝙝𝙖𝙩"
CH_BIRTHDAY = "🎂・Cumpleaños"
CH_LEVELS = "💯・Niveles"
VOICE_COMMUNITY = "🔊・𝙜𝙚𝙣𝙚𝙧𝙖𝙡"

CH_DRAWINGS = "🎨・Dibujos"
CH_MEMES = "😂・𝙢𝙚𝙢𝙚𝙨-𝙮-𝙘𝙡𝙞𝙥𝙨"
CH_CLIPS = "🎞️・𝙘𝙡𝙞𝙥𝙨"

CH_FRIENDS_CHAT = "💬・𝙘𝙝𝙖𝙩-𝙖𝙢𝙞𝙜𝙤𝙨"
CH_FRIENDS_MUSIC = "🎵・Música"
VOICE_FRIENDS = "🔊・𝙨𝙖𝙡𝙖-𝙙𝙚-𝙖𝙢𝙞𝙜𝙤𝙨"
...
CH_ARK_CHAT = "💬・ark"
CH_ARK_PATCHES = "🛠️・ark-parches"
CH_ARK_MUSIC = "🎵・Música"
VOICE_ARK = "🔊 General"

CH_GTA_CHAT = "💬・gta"
CH_GTA_CLIPS = "📸・gta-clips"
CH_GTA_CARS = "🏎️・coches-y-tuning"
VOICE_GTA = "🔊 GTA"

CH_BOT_COMMANDS = "🤖・𝙘𝙤𝙢𝙖𝙣𝙙𝙤𝙨-𝙗𝙤𝙩"
CH_BOT_MUSIC = "🎵・Música-Bot"
CH_BOT_LEVELS = "📊・Niveles-Bot"
CH_BOT_SETUP = "🧰・Config-Bots"

CH_VIP_CHAT = "💎・vip-chat"
CH_VIP_GIVEAWAYS = "🎁・sorteos-vip"
VOICE_VIP = "🔊・VIP LOUNGE"

CH_STAFF_CHAT = "🛡️・𝙨𝙩𝙖𝙛𝙛"
CH_STAFF_CONTROL = "⚙️・Control-Staff"
CH_WARNS = "🚨・Warns"
CH_LOGS = "📋・𝙡𝙤𝙜𝙨"
CH_DATA = "🗄️・𝙙𝙖𝙩𝙤𝙨-𝙗𝙤𝙩"
VOICE_STAFF = "🔊 Staff"
...
async def migrate_existing(guild):
    ...
    for target, aliases in CATEGORY_ALIASES.items():
        ...
        for old in aliases:
            ...
            if cat:
                try:
                    await cat.edit(name=target, reason="ARMENKR V2 migration")
                    break
        ...
        for target_name, aliases in mappings.items():
            ...
            for ch in list(cat.channels):
                if ch.name in aliases:
                    try:
                        await ch.edit(name=target_name, reason="ARMENKR V2 migration")
                        break
                    except discord.HTTPException:
                        pass
    ...
    if server and staff:
        ...
        if warns:
            try:
                await warns.edit(category=staff, reason="ARMENKR V2: warns privado")
            except discord.HTTPException:
                pass
...
async def ensure_category(guild, name):
    ...
    if name == CAT_STAFF:
        ...
        ow = role_gate_overwrites(guild, ROLE_VIP)
    else:
        ow = public_chat_overwrites(guild)

    return await guild.create_category(name=name, overwrites=ow, reason="ARMENKR V2 setup")
...
async def ensure_channel(guild, cat, kind, name):
    if kind == "text":
        ...
        kwargs = dict(name=name, category=cat, reason="ARMENKR V2 setup")
    ...
    return await guild.create_voice_channel(
        name=name, category=cat, reason="ARMENKR V2 setup"
    )
...
async def enforce_permissions(guild):
    ...
    for name in [CH_WELCOME, CH_RULES, CH_SOCIALS, CH_LIVE, CH_ANNOUNCE]:
        ...
        if ch:
            try:
                await ch.edit(overwrites=readonly_overwrites(guild), reason="ARMENKR V2 permissions")
            except discord.HTTPException:
                pass
    ...
    if roles_ch:
        ...
        try:
            await roles_ch.edit(overwrites=ow, reason="ARMENKR V2 role selector")
        except discord.HTTPException:
            pass

    # Público
    for name in [CH_CHAT, CH_BIRTHDAY, CH_LEVELS, CH_DRAWINGS, CH_MEMES, CH_CLIPS,
                 CH_BOT_COMMANDS, CH_BOT_MUSIC, CH_BOT_LEVELS, CH_BOT_SETUP]:
        ch = text_by_name(guild, name)
        if ch:
            try:
                await ch.edit(overwrites=public_chat_overwrites(guild), reason="ARMENKR V2 public")
            except discord.HTTPException:
                pass
    ...
    for cat_name, role_name in [
        ...
        if cat:
            try:
                await cat.edit(overwrites=role_gate_overwrites(guild, role_name), reason="ARMENKR V2 gated")
            except discord.HTTPException:
                pass
            for ch in cat.channels:
                try:
                    await ch.edit(sync_permissions=True, reason="ARMENKR V2 sync")
                except discord.HTTPException:
                    pass
    ...
    if staff:
        try:
            await staff.edit(overwrites=staff_overwrites(guild), reason="ARMENKR V2 staff")
        except discord.HTTPException:
            pass
        for ch in staff.channels:
            if isinstance(ch, discord.TextChannel) and ch.name == CH_DATA:
                try:
                    await ch.edit(overwrites=data_overwrites(guild), reason="ARMENKR V2 data")
                except discord.HTTPException:
                    pass
            else:
                try:
                    await ch.edit(sync_permissions=True, reason="ARMENKR V2 staff sync")
                except discord.HTTPException:
                    pass
...
async def organize(guild):
    desired = [
        ...
        CAT_MC, CAT_ARK, CAT_GTA, CAT_BOTS, CAT_VIP, CAT_STAFF,
    ]
    for pos, name in enumerate(desired):
        cat = category_by_name(guild, name)
        if cat:
            try:
                await cat.edit(position=pos, reason="ARMENKR V2 organization")
            except discord.HTTPException:
                pass
    ...
    for cat_name, channels in BLUEPRINT:
        ...
        for pos, (_, name) in enumerate(channels):
            ...
            if ch:
                try:
                    await ch.edit(position=pos, reason="ARMENKR V2 order")
                except discord.HTTPException:
                    pass
...
async def refresh_panels(guild):
    ...
    # Ranking / actividad
    await refresh_activity_panel(guild)

    # Bots recomendados
    ch = text_by_name(guild, CH_BOT_SETUP)
    if ch:
        embed = discord.Embed(
            title="🧰 BOTS PARA TERMINAR EL SERVIDOR",
            description=(
                "El bot **ARMENKR** ya se encarga de estructura, roles, bienvenida, "
                "redes, ranking, VIP/OG y avisos manuales de directo.\n\n"
                "Para no duplicar funciones, estos son los únicos bots externos que vale la pena añadir:"
            ),
            color=ACCENT,
        )
        embed.add_field(
            name="🔴 Streamcord",
            value="Avisos automáticos cuando entres en directo. Úsalo para reemplazar el aviso manual si quieres automatización total.",
            inline=False,
        )
        embed.add_field(
            name="📊 Statbot",
            value="Analítica del servidor: actividad, voz, crecimiento y canales más usados.",
            inline=False,
        )
        embed.add_field(
            name="📅 Sesh",
            value="Eventos de comunidad: GTA, ARK, Minecraft, FC y noches con viewers.",
            inline=False,
        )
        embed.add_field(
            name="🛡️ Carl-bot",
            value="AutoMod y logs. No actives bienvenida ni reaction roles porque ARMENKR ya los maneja.",
            inline=False,
        )
        embed.set_footer(text="ARMENKR • Menos bots, mejor configurados.")
        await upsert_embed(ch, "ARMENKR_PANEL_BOT_SETUP", embed)

    # Control staff
    ch = text_by_name(guild, CH_STAFF_CONTROL)
    if ch:
        embed = discord.Embed(
            title="⚙️・𝘾𝙊𝙉𝙏𝙍𝙊𝙇 𝙎𝙏𝘼𝙁𝙁",
            description=(
                "`/organizar` — estructura + permisos\n"
                "`/paneles` — refresca paneles\n"
                "`/envivo` — publica un stream\n"
                "`/configurarredes` — configura enlaces\n"
                "`/darvip` / `/quitarvip` — acceso VIP\n"
                "`/darog` / `/quitarog` — rol OG\n"
                "`/actualizartop` — recalcula Top Community\n"
...
# =========================================================
# ACTIVIDAD / TOP COMMUNITY
# =========================================================

ACTIVITY_PREFIX = "ARMENKR_ACTIVITY:"
activity_cache = {}
...
MESSAGE_POINT_COOLDOWN = 60
VOICE_POINT_BLOCK_MINUTES = 5
TOP_COMMUNITY_SIZE = 5
TOP_COMMUNITY_MIN_POINTS = 10


def blank_activity():
    return {"points": 0, "messages": 0, "voice_minutes": 0}


async def load_activity_cache(guild):
    ch = text_by_name(guild, CH_DATA)
    if not ch:
        return

    activity_cache.clear()
    activity_message_ids.clear()

    async for msg in ch.history(limit=1000):
        if msg.author != guild.me or not msg.content.startswith(ACTIVITY_PREFIX):
            continue

        try:
            prefix, raw = msg.content.split(" ", 1)
            user_id = int(prefix[len(ACTIVITY_PREFIX):])
            data = json.loads(raw)
            if not isinstance(data, dict):
                continue

            activity_cache[user_id] = {
                "points": int(data.get("points", 0)),
                "messages": int(data.get("messages", 0)),
                "voice_minutes": int(data.get("voice_minutes", 0)),
            }
            activity_message_ids[user_id] = msg.id
        except Exception:
            continue


async def save_activity_user(guild, user_id):
    ch = text_by_name(guild, CH_DATA)
    if not ch:
        return

    data = activity_cache.get(user_id, blank_activity())
    payload = f"{ACTIVITY_PREFIX}{user_id} " + json.dumps(
        data, ensure_ascii=False, separators=(",", ":")
    )

    msg_id = activity_message_ids.get(user_id)
    if msg_id:
        try:
            ...
            activity_message_ids.pop(user_id, None)

    msg = await ch.send(payload)
    activity_message_ids[user_id] = msg.id


async def flush_activity(guild):
    if not activity_dirty:
        return

    pending = list(activity_dirty)
    activity_dirty.clear()

    for user_id in pending:
        try:
            await save_activity_user(guild, user_id)
            await asyncio.sleep(0.05)
        except Exception as exc:
            print(f"❌ Error guardando actividad {user_id}: {exc}")
            activity_dirty.add(user_id)


def get_activity(user_id):
    if user_id not in activity_cache:
        activity_cache[user_id] = blank_activity()
    return activity_cache[user_id]


def sorted_activity_members(guild):
    rows = []
    for user_id, data in activity_cache.items():
        member = guild.get_member(user_id)
        if not member or member.bot:
            continue
        rows.append((member, data))
    rows.sort(key=lambda x: (x[1].get("points", 0), x[1].get("messages", 0)), reverse=True)
    return rows


async def update_top_community(guild):
    role = role_by_name(guild, ROLE_TOP)
    if not role:
        return

    ranking = [
        member for member, data in sorted_activity_members(guild)
        if data.get("points", 0) >= TOP_COMMUNITY_MIN_POINTS
    ][:TOP_COMMUNITY_SIZE]

    winners = {m.id for m in ranking}
    ...
    for member in guild.members:
        ...
        should_have = member.id in winners

        try:
            if should_have and not has_role:
                await member.add_roles(role, reason="ARMENKR Top Community automático")
            elif has_role and not should_have:
                await member.remove_roles(role, reason="ARMENKR Top Community actualizado")
        except discord.HTTPException:
            pass


async def refresh_activity_panel(guild):
    ch = text_by_name(guild, CH_LEVELS)
    if not ch:
        return

    ranking = sorted_activity_members(guild)[:10]

    if ranking:
        lines = []
        medals = ["🥇", "🥈", "🥉"]
        for i, (member, data) in enumerate(ranking, start=1):
            prefix = medals[i - 1] if i <= 3 else f"`#{i}`"
            lines.append(
                f"{prefix} {member.mention} — **{data.get('points', 0)} pts** "
                f"• 💬 {data.get('messages', 0)} • 🎙️ {data.get('voice_minutes', 0)} min"
            )
        description = "\n".join(lines)
    else:
        description = "Todavía no hay actividad suficiente para mostrar el ranking."
    ...
    embed.add_field(
        name="¿Cómo sumo puntos?",
        value=(
            "💬 Participando en chats sin hacer spam.\n"
            "🎙️ Pasando tiempo en canales de voz.\n\n"
            f"Los **Top {TOP_COMMUNITY_SIZE}** activos reciben automáticamente `{ROLE_TOP}`."
        ),
        inline=False,
    )
    embed.set_footer(text="ARMENKR • La actividad real vale más que el spam.")

    await upsert_embed(ch, "ARMENKR_PANEL_ACTIVITY", embed)


async def group_existing_bots(guild):
    ...
    for member in guild.members:
        if member.bot and role not in member.roles:
            try:
                await member.add_roles(role, reason="ARMENKR: agrupar bots")
            except discord.HTTPException:
                pass


@tasks.loop(minutes=5)
async def activity_flush_loop():
    await bot.wait_until_ready()
    guild = bot.get_guild(GUILD_ID)
    if guild:
        await flush_activity(guild)


@tasks.loop(minutes=15)
async def community_rank_loop():
    await bot.wait_until_ready()
    guild = bot.get_guild(GUILD_ID)
    if guild:
        await update_top_community(guild)
        await refresh_activity_panel(guild)
...
class ArmenBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        if not activity_flush_loop.is_running():
            activity_flush_loop.start()
        if not community_rank_loop.is_running():
            community_rank_loop.start()
...
# AKR Content HQ integrado: un solo bot para streamer + contenido FC27.
from akr_content import register_akr_content
register_akr_content(bot, GUILD_ID)
...
@app_commands.command(name="organizar", description="Comando legado: usa /setup_discord_final.")
async def organizar_cmd(interaction: discord.Interaction):
    if not isinstance(interaction.user, discord.Member) or not interaction.user.guild_permissions.administrator:
        ...
        return
    await interaction.response.send_message(
        "i️ El organizador antiguo quedó desactivado para no volver a llenar el Discord. "
        "Usa **/setup_discord_final** para aplicar el diseño final limpio de ARMENKR.",
        ephemeral=True,
    )
...
async def configurarredes(
    ...
    if not changed:
        await interaction.followup.send("i️ No enviaste ningún enlace.", ephemeral=True)
        return
...
@app_commands.guild_only()
@app_commands.command(name="ranking", description="Muestra el Top Community actual.")
async def ranking(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=False)
    guild = interaction.guild
    ranking_rows = sorted_activity_members(guild)[:10]

    if not ranking_rows:
        await interaction.followup.send("Todavía no hay actividad suficiente para mostrar el ranking.")
        return

    medals = ["🥇", "🥈", "🥉"]
    lines = []
    for i, (member, data) in enumerate(ranking_rows, start=1):
        prefix = medals[i - 1] if i <= 3 else f"`#{i}`"
        lines.append(
            f"{prefix} {member.mention} — **{data.get('points', 0)} pts** "
            f"• 💬 {data.get('messages', 0)} • 🎙️ {data.get('voice_minutes', 0)} min"
...
@app_commands.guild_only()
@app_commands.command(name="mipuntos", description="Muestra tus puntos de comunidad.")
async def mipuntos(interaction: discord.Interaction):
    data = get_activity(interaction.user.id)
    embed = discord.Embed(
        title=f"🏆 Actividad de {interaction.user.display_name}",
        color=ACCENT,
    )
    embed.add_field(name="Puntos", value=str(data.get("points", 0)), inline=True)
    embed.add_field(name="Mensajes válidos", value=str(data.get("messages", 0)), inline=True)
    embed.add_field(name="Voz", value=f"{data.get('voice_minutes', 0)} min", inline=True)
    await interaction.response.send_message(embed=embed, ephemeral=True)


async def _toggle_staff_role(interaction, jugador, role_name, add):
    if not isinstance(interaction.user, discord.Member) or not is_staff(interaction.user):
        await interaction.response.send_message("⛔ Solo staff.", ephemeral=True)
        return
    ...
    role = role_by_name(interaction.guild, role_name)
    if not role:
        await interaction.response.send_message("❌ Ese rol no existe. Ejecuta `/organizar`.", ephemeral=True)
        return

    try:
        if add:
            await jugador.add_roles(role, reason=f"ARMENKR staff: {interaction.user}")
            action = "agregado"
        else:
            await jugador.remove_roles(role, reason=f"ARMENKR staff: {interaction.user}")
            action = "quitado"

        await interaction.response.send_message(
            f"✅ {role.mention} {action} para {jugador.mention}.",
            ephemeral=True,
        )
    except discord.Forbidden:
        await interaction.response.send_message(
            "❌ No puedo administrar ese rol. Sube el rol del bot por encima de VIP/OG.",
            ephemeral=True,
        )


@app_commands.guild_only()
@app_commands.command(name="darvip", description="Da acceso VIP a un miembro.")
async def darvip(interaction: discord.Interaction, jugador: discord.Member):
    await _toggle_staff_role(interaction, jugador, ROLE_VIP, True)


@app_commands.guild_only()
@app_commands.command(name="quitarvip", description="Quita el acceso VIP a un miembro.")
async def quitarvip(interaction: discord.Interaction, jugador: discord.Member):
    await _toggle_staff_role(interaction, jugador, ROLE_VIP, False)


@app_commands.guild_only()
@app_commands.command(name="darog", description="Da el rol OG a un miembro.")
async def darog(interaction: discord.Interaction, jugador: discord.Member):
    await _toggle_staff_role(interaction, jugador, ROLE_OG, True)


@app_commands.guild_only()
@app_commands.command(name="quitarog", description="Quita el rol OG a un miembro.")
async def quitarog(interaction: discord.Interaction, jugador: discord.Member):
    await _toggle_staff_role(interaction, jugador, ROLE_OG, False)


@app_commands.guild_only()
@app_commands.command(name="actualizartop", description="Recalcula el rol Top Community.")
async def actualizartop(interaction: discord.Interaction):
    if not isinstance(interaction.user, discord.Member) or not is_staff(interaction.user):
        await interaction.response.send_message("⛔ Solo staff.", ephemeral=True)
        return

    await interaction.response.defer(ephemeral=True)
    await flush_activity(interaction.guild)
    await update_top_community(interaction.guild)
    await refresh_activity_panel(interaction.guild)
    await interaction.followup.send("✅ Top Community actualizado.", ephemeral=True)
...
bot.tree.add_command(mipuntos)
bot.tree.add_command(darvip)
bot.tree.add_command(quitarvip)
bot.tree.add_command(darog)
bot.tree.add_command(quitarog)
bot.tree.add_command(actualizartop)
...
async def on_message(message: discord.Message):
    ...
    last = message_cooldowns.get(message.author.id, 0)

    if now - last >= MESSAGE_POINT_COOLDOWN:
        message_cooldowns[message.author.id] = now
        data = get_activity(message.author.id)
        data["points"] = data.get("points", 0) + 1
        data["messages"] = data.get("messages", 0) + 1
        activity_dirty.add(message.author.id)
...
async def on_voice_state_update(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
    ...
    if before.channel is None and after.channel is not None:
        voice_join_times[member.id] = loop_time
        return

    # Cambió de canal o salió: contabiliza la sesión anterior
    if before.channel is not None and before.channel != after.channel:
        started = voice_join_times.pop(member.id, None)
        if started is not None:
            elapsed_minutes = max(0, int((loop_time - started) // 60))
            if elapsed_minutes > 0:
                data = get_activity(member.id)
                data["voice_minutes"] = data.get("voice_minutes", 0) + elapsed_minutes
                data["points"] = data.get("points", 0) + (elapsed_minutes // VOICE_POINT_BLOCK_MINUTES)
                activity_dirty.add(member.id)

        # Si cambió a otro canal, comienza nueva sesión
...
async def on_ready():
    print("=" * 60)
    print(f"👑 ARMENKR + AKR BOT conectado como {bot.user}")
    print("=" * 60)
    ...
    # IMPORTANTE: ya no reorganiza todo el servidor automáticamente al reiniciar.
    # /organizar conserva el setup antiguo si algún día lo necesitas.
    # /setup_discord_final aplica el diseño final limpio cuando tú lo decidas.
    try:
        await load_activity_cache(guild)
        await update_top_community(guild)
        await refresh_activity_panel(guild)
        print("✅ ARMENKR FINAL listo. No se hicieron cambios estructurales automáticos.")
    except Exception as exc:
        print(f"⚠️ Inicio parcial: {type(exc).__name__}: {exc}")
